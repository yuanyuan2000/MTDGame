...
class Network:
    ...
    def test(self):
        ...
        # print(dict(nx.get_node_attributes(self.graph, "host")))
        # print(dict(nx.get_node_attributes(self.graph, "subnet")))
        # print(dict(nx.get_node_attributes(self.graph, "layer")))
        print('------------------------')
    ...
    def gen_graph(self, min_nodes_per_subnet=4, max_subnets_per_layer=5, subnet_m_ratio=0.2,
        ...
        # Add one to random layer until there is the correct amount of subnets (Could be Optimised in future)
        ...
        # Add one to random layer until there is the correct number of nodes (Could be Opitmised in future)
        ...
        for i, subnet_node_list in enumerate(subnet_nodes):
            for j, s_nodes in enumerate(subnet_node_list):
                ...
                if i != 0:
                    # subgraph_pos = {
                    #     k: np.array(
                    #         [v[0] + i * 0.45, v[1] + j * 0.6 + 0.25 * (max_subnet_in_layer - len(subnet_node_list))])
        ...
        for n in range(self.total_endpoints):
            position = (n + 1) / self.total_endpoints * (self.max_y_pos - self.min_y_pos) + self.min_y_pos
    ...
    def get_hosts(self):
        # self.graph.nodes.get(host_id, {}).get("host", None)
